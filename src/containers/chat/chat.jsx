...
class Chat extends Component {
  ...
  componentWillMount () {
    ...
    const emojis = ['😀', '😁', '🤣','😀', '😁', '🤣','😀', '😁', '🤣','😀', '😁', '🤣','😀'
      ...
      ,'😁', '🤣','😀', '😁', '🤣','😀', '😁', '🤣','😀', '😁', '🤣'
      ,'😁', '🤣','😀', '😁', '🤣','😀', '😁', '🤣','😀', '😁', '🤣']
    this.emojis = emojis.map(emoji => ({text: emoji}))//将表情数组全存在对象里
    ...
    this.animals = animals.map(animal => ({text: animal}))//将表情数组全存在对象里
  }
  ...
  toggleShow = () => {
    ...
    this.setState({isShow,isShowAnimal:false})
    if(isShow) {
  ...
  toggleShowAnimal = () => {
    const isShowAnimal = !this.state.isShowAnimal
    this.setState({isShowAnimal,isShow:false})
    if(isShowAnimal) {
      // 异步手动派发resize事件,解决表情列表显示的bug
      setTimeout(() => {
        window.dispatchEvent(new Event('resize'))
        ...
        <div id='chat-page'>
          ...
          <div className='am-tab-bar'>
            <InputItem
                ...
                onFocus={() => this.setState({isShow:false,isShowAnimal:false})}
                onKeyUp={this.keyupSubmit}
                extra={
                  <span>
                    <span onClick={this.toggleShow} style={{marginRight:10}}>😊</span>
                    <span onClick={this.toggleShowAnimal} style={{marginRight:10}}>🙈</span>
